...
def create_sarimax_df(df, df_ext=None, lags=[24*365], lags_ext=[1,24,365*24], freqs=[24, 24*365], levels=None, add_const=False):
...
# df_solar = df_solar.iloc[-24*365*3:]
# df_eolica = df_eolica.iloc[-24*365*3:]
...
X_train_solar = create_sarimax_df(Y_full_train_solar, Y_full_train_eolica, lags=LAGS, lags_ext=LAGS_EXT, freqs=FREQS, levels=LEVELS, add_const=True)
Y_train_solar = X_train_solar[['y']]
X_train_solar = X_train_solar.drop('y',axis=1)
model_solar = OLS(Y_train_solar['y'], exog=X_train_solar)
results_solar = model_solar.fit()

X_train_eolica = create_sarimax_df(Y_full_train_eolica, Y_full_train_solar, lags=LAGS, lags_ext=LAGS_EXT, freqs=FREQS, levels=LEVELS, add_const=True)
Y_train_eolica = X_train_eolica[['y']]
X_train_eolica = X_train_eolica.drop('y',axis=1)
model_eolica = OLS(Y_train_eolica['y'], exog=X_train_eolica)
results_eolica = model_eolica.fit()
...
for i in range(n_laps):
    ...
    if LAGS_EXT is not None:
        assert horizon <= min(LAGS_EXT)

    future_Y_solar = extend_df(Y_full_train_solar, pd.Series(np.ones(horizon)))
    X_train_solar = create_sarimax_df(future_Y_solar, df_ext=Y_full_train_eolica, lags=LAGS, lags_ext=LAGS_EXT, freqs=FREQS, levels=LEVELS, add_const=True)
    X_train_solar = X_train_solar.drop('y',axis=1)
    forecast = results_solar.get_prediction(exog=X_train_solar.iloc[-horizon:])
    ...
    X_train_eolica = create_sarimax_df(future_Y_eolica, df_ext=Y_full_train_solar, lags=LAGS, lags_ext=LAGS_EXT, freqs=FREQS, levels=LEVELS, add_const=True)
    X_train_eolica = X_train_eolica.drop('y',axis=1)
    forecast = results_eolica.get_prediction(exog=X_train_eolica.iloc[-horizon:])
    forecast = forecast.predicted_mean
...
ax.grid()
plt.savefig('regression_solar.png', dpi=300, bbox_inches='tight')
...
ax.legend(['Real','Prediction'])
ax.grid()
plt.savefig('regression_eolica.png', dpi=300, bbox_inches='tight')

# Save results
timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
save_solar = f'sarimax_solar_{timestamp}.csv'
save_eolica = f'sarimax_eolica_{timestamp}.csv'
Y_full_train_solar.to_csv(f'./results/{save_solar}')
Y_full_train_eolica.to_csv(f'./results/{save_eolica}')

with open(f'./results/model_regression_solar_{timestamp}.pkl','wb') as f:
    pkl.dump(results_solar, f)
with open(f'./results/model_regression_eolica_{timestamp}.pkl','wb') as f:
    pkl.dump(results_eolica, f)