...
model_solar = ARIMA(Y_train_solar['y'], exog=X_train_solar, order=ORDER)
...
model_eolica = ARIMA(Y_train_eolica['y'], exog=X_train_eolica, order=ORDER)
results_eolica = model_eolica.fit()


print("#### Starting SARIMAX prediction")
...
for i in range(n_laps):
    ...
    X_train_solar = X_train_solar.drop('y',axis=1)
    forecast = results_solar.get_forecast(steps=horizon, exog=X_train_solar.iloc[-horizon:])
    forecast = forecast.predicted_mean
    Y_full_train_solar = extend_df(Y_full_train_solar, forecast)
    X_train_solar = create_sarimax_df(Y_full_train_solar, df_ext=Y_full_train_eolica, lags=LAGS, lags_ext=LAGS_EXT, freqs=FREQS, levels=LEVELS)
    X_train_solar = X_train_solar.drop('y',axis=1)
    results_solar = results_solar.append(Y_full_train_solar.iloc[-horizon:], exog=X_train_solar.iloc[-horizon:], refit=False)
    ...
    X_train_eolica = X_train_eolica.drop('y',axis=1)
    forecast = results_eolica.get_forecast(steps=horizon, exog=X_train_eolica.iloc[-horizon:])
    forecast = forecast.predicted_mean
    Y_full_train_eolica = extend_df(Y_full_train_eolica, forecast)
    X_train_eolica = create_sarimax_df(Y_full_train_eolica, df_ext=Y_full_train_solar, lags=LAGS, lags_ext=LAGS_EXT, freqs=FREQS, levels=LEVELS)
    X_train_eolica = X_train_eolica.drop('y',axis=1)
    results_eolica = results_eolica.append(Y_full_train_eolica.iloc[-horizon:], exog=X_train_eolica.iloc[-horizon:], refit=False)
...
ax.grid()
plt.savefig('sarimax_solar.png', dpi=300, bbox_inches='tight')
...
ax.legend(['Real','Prediction'])
ax.grid()
plt.savefig('sarimax_eolica.png', dpi=300, bbox_inches='tight')
...
with open(f'./results/model_sarimax_solar_{timestamp}.pkl','wb') as f:
    pkl.dump(results_solar, f)
with open(f'./results/model_sarimax_eolica_{timestamp}.pkl','wb') as f:
